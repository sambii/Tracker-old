...
errors_count = <%= @attendance_type.errors.count %>
...
if errors_count.toString() != '0'
  ...
  # $('#modal_content').html("<%= escape_javascript(render('attendance_types/edit') ) %>");
  $('#modal-body').html("<%= escape_javascript(render('attendance_types/edit') ) %>");
else
