...
NUM_TEACHERS = 14 # number of teachers per subject
SECTS_PER_TEACHER = 2
...
namespace :stem_egypt_training_data do
  ...
  task load_training_school: :environment do
    ...
    # # This creates a school with school year

    # school = School.new()
    # school.name = "Stem Egypt Training High School"
    # school.acronym = "ETH"
    # school.street_address = "101 W. Elm St."
    # school.city = "Conshohocken"
    # school.state = "PA"
    # school.zip_code = "19428"
    # school.marking_periods = 2
    # # school.add_flag(School::USE_FAMILY_NAME)
    # # school.add_flag(School::USER_BY_FIRST_LAST)
    # # school.add_flag(School::GRADE_IN_SUBJECT_NAME)
    # if !school.save
    ...
    school_year.name = "2015-16"
    school_year.school_id = school.id
    school_year.starts_at = Date.parse("2015-09-01")
    school_year.ends_at = Date.parse("2016-06-21")
    if !school_year.save
      puts "!!!!!/nERROR: Create school got error: #{school.errors.full_messages}/n!!!!!"
      next
    end
    ...
    school_admin.email = 'tayloredwebsites@me.com'
    ...
    if !counselor.save
      puts "!!!!!/nERROR: create counselor error #{counselor.errors.full_messages}/n!!!!!"
      next
    end
  ...
  task create_training_los: :environment do
    ...
    all_los = [
      ...
      ['Arabic', 1, 1, '', "AR.1.21 - Applying critical analysis and criticism to work of prose and express a critical opinion using evidence to support their opinions.3. نقد الأعمال القصصية وتحليلها وابدء رأى نقدى بناء مع ايجاد براهين لذلك الرأى."],
      ['Arabic', 1, 1, '', "AR.1.22 - Explore the writer’s personal and literary characteristics and the influence of the surrounding environment for each literary work.6. إستنباط خصائص أسلوب الأديب وسمات شخصيته وأثر البيئة المحيطة من كل عمل أدبى."],
      ['Arabic', 1, 1, '', "AR.1.23 - Mastering writing creative literary work of different literary topics to reflect the student’s ability to absorb the major challenges facing Egypt using the rules of the language.10. التمكن من إبداع موضوعات أدبية مختلفة تعكس مدى قدرته على إستي"],
      ['Arabic', 1, 1, '', "AR.1.24 - Applying Rhetorical Analysis to literary and poetic texts that deal with the emotions and the literary direction of the writer.2. تحليل النص الأدبى النثرى أو الشعرى تحليلا بلاغيا يتناول عاطفة الاديب واتجاهه الأدبى والصور البلاغية وعلم البديع و"],
      ['Arabic', 1, 1, '', "AR.1.25 - Excelling in giving speeches, reading aloud, and discussion, taking into account the correct pronunciation, signals, and intonation for persuasion.9. البراعة فى الإلقاء أو القراءة الجهرية أو المناقشة مع مراعاة سلامة النطق والإشارة وتنغيم الصوت"],
      ['Arabic', 1, 1, '', "AR.1.26 - Applying grammatical rules during speaking, discussion and presentations to apply what they were taught.1. تطبيق القواعد النحوية \اثناء الحديث والمجادلة العرض التقديمى بحيث يوظف ما درسه عمليا."],
      ...
      ['Biology', 1, 1, '', "BI.1.11 - Create your own experiment to investigate a factor that affects photosynthesis and/or respiration
      Use the steps of experimental design"],
      ['Biology', 1, 1, '', "BI.1.12 - Create a model that shows the interdependence of living organisms within an ecosystem
      ...
      Conside"],
      ['Biology', 1, 1, '', "BI.1.13 - Connect the cycling of carbon to global climate change
      ...
      •Ef"],
      ['Biology', 1, 1, '', "BI.1.14 - Analyze how natural and human caused events can unbalance populations within an ecosystem and make a judgments about the ways to rebalance the ecosystem
      • Describe natural disasters which cause unbalancing of ecosystems.
      • Describe human caused"],
      ['Biology', 1, 1, '', "BI.1.15 - Analyze an ecosystem in Egypt that has become unbalanced and suggest effective interventions"],
      ...
      ['Chemistry', 1, 1, '', "CH.1.11 - Students must calculate quantities of products formed from known quantities of reactants and be able to discuss their precision and accuracy."],
      ['Chemistry', 1, 1, '', "CH.1.12 - Investigate four types of chemical reactions, generate and test for hydrogen, oxygen and carbon dioxide and determine the most effective ratio of hydrogen to oxygen for propulsion of a small rocket."],
      ['Chemistry', 1, 1, '', "CH.1.13 - Students will use their understanding of the metal activity series to explain why metals are found as they are in nature and discuss considerations (such as exposure to different kinds of solutions) for the use of metals in industry, constructio"],
      ['Chemistry', 1, 1, '', "CH.1.14 - Students will use two and three dimensional models and their understanding of bond polarity to illustrate polar and non-polar inter-molecular forces."],
      ['Chemistry', 1, 1, '', "CH.1.15 - Students will examine a variety of commercial batteries and use their understanding of electrochemistry to explain how they work and why the manufacturers used the materials they did."],
      ['Chemistry', 1, 1, '', "CH.1.16 - Students will determine, explain and illustrate how energy and disorder change during physical and chemical processes."],
      ...
      ['Earth Science', 1, 1, '', "ES.1.08 - Students will analyze and identify common ore minerals that are used as a resource for modern industries."],
      ['Earth Science', 1, 1, '', "ES.1.09 - Students will understand the different resources used by different countries to meet their energy needs."],
      ['Earth Science', 1, 1, '', "ES.1.10 - Students will be able to recognize the processes by which fossil fuels (coal) are extracted and processed for human use."],
      ['Earth Science', 1, 1, '', "ES.1.11 - Students will be able to recognize the processes by which fossil fuels (petroleum and natural gas) are extracted and processed for human use."],
      ['Earth Science', 1, 1, '', "ES.1.12 - Students will be able to evaluate environmental impacts of fossil fuel resource use and suggest innovative alternatives."],
      ['Earth Science', 1, 1, '', "ES.1.13 - Students will evaluate potential renewable energy sources in Egypt to replace dependence upon fossil fuels."],
      ['English', 1, 1, '', "EN.1.02 - Students will be able to identify gist and main ideas."],
      ['English', 1, 1, '', "EN.1.03 - Students will be able to reflect on ideas"],
      ['English', 1, 1, '', "EN.1.04 - Students will be able to distinguish among facts and speculation in a text."],
      ['English', 1, 1, '', "EN.1.05 - Students will be able to use technology to produce, publish and update individual or shared writing products."],
      ['English', 1, 1, '', "EN.1.06 - Students will be able to produce clear and coherent writing, in which the development, organization, and style are appropriate to task, purpose and audience."],
      ['English', 1, 1, '', "EN.1.07 - Students will be able to read, comprehend and appreciate the values, beliefs and practices of both national and target cultures."],
      ['English', 1, 1, '', "EN.1.08 - Students will be able to interpret a work of literature and relate the information to contemporary"],
      ['English', 1, 1, '', "EN.1.09 - Students will be able to participate effectively in discussions and debates (one-on-one, in groups, and teacher-led) with diverse topics and issues."],
      ['English', 1, 1, '', "EN.1.10 - Students will be able to determine the meaning of words and phrases as they are used in the text."],
      ['English', 1, 1, '', "EN.1.11 - Students will be able to identify and correctly use idioms, phrasal verbs, collocations and affixes."],
      ['English', 1, 1, '', "EN.1.12 - Students will be able to use appropriate vocabulary to describe people, places, events and experiments."],
      ...
      ['French', 1, 1, '', "FR.1.05 - Etre capable d'employer les verbs (avoir, aller, arriver, habiter)"],
      ['French', 1, 1, '', "FR.1.06 - Distinguer entre les femenin et le masculin"],
      ['French', 1, 1, '', "FR.1.07 - Nommer les traits du visage, les couleurs, les vetements, les professions et les membre de la familles inclus dans le livre"],
      ['French', 1, 1, '', "FR.1.08 - Etre capable de s'informer sur les caracters d'une personne"],
      ['French', 1, 1, '', "FR.1.09 - Nommer les qualites et les defauts."],
      ['French', 1, 1, '', "FR.1.10 - Nommer les signes de l'horiscope et les astres"],
      ['French', 1, 1, '', "FR.1.11 - Nommer les 4 saisons de l'annee, les mois de l'annee et les points cardinaux"],
      ['French', 1, 1, '', "FR.1.12 - Nommer les pieces de la maison et les meubles (inclus dans le livre)"],
      ['French', 1, 1, '', "FR.1.13 - Preciser le lieu"],
      ['German', 1, 1, 'DE.1.01', "Die Schüler können (1-Etwas buchstabieren, 2-eine Visitenkarte lesen, 3-ein Anmeldeformular ausfüllen)"],
      ['German', 1, 1, 'DE.1.04', "Die Schüler können (1. von 21 - bis 100 zählen"],
      ['German', 1, 1, 'DE.1.05', "Die Schüler können 3.einen Zeitungsartikel verstehen)"],
      ['German', 1, 1, 'DE.1.02', "Schüler können  2.ein Formular ausfüllen,"],
      ['German', 1, 1, 'DE.1.03', "Schüler können 3.sich und andere Vorstellen)"],
      ...
      ['Math', 1, 1, '', "MA.1.07 - Create, interpret and analyze quadratic functions that model real-world situations."],
      ['Math', 1, 1, '', "MA.1.08 - Create, interpret and analyze exponential and logarithmic functions that model real-world situations."],
      ['Math', 1, 1, '', "MA.1.09 - Create, interpret and analyze trigonometric functions that model real-world situations."],
      ['Math', 1, 1, '', "MA.1.10 - Prove and apply trigonometric identities"],
      ['Math', 1, 1, '', "MA.1.11 - Create, interpret and analyze systems of linear functions that model real-world situations."],
      ['Math', 1, 1, '', "MA.1.12 - Apply determinants and their properties in real-world situations"],
      ...
      ['Mechanics', 1, 1, '', "ME.1.06 - Students will be able to use Newtown's 1st Law and vector algebra to analyze systems in translational equilibrium."],
      ['Mechanics', 1, 1, '', "ME.1.07 - Students will experimentally justify Newton's 2nd law and use it to mathematically predict motion of objects in 1-D"],
      ['Mechanics', 1, 1, '', "ME.1.08 - Students will be able to use Newtown's 2nd Law and vector algebra to analyze motion of objects in 2-D"],
      ['Mechanics', 1, 1, '', "ME.1.09 - Students will be able to analyze circular motion of an object using concepts of centripetal acceleration and centripetal force"],
      ...
      ['Physics', 1, 1, '', "PH.1.06 - Students will understand that certain material objects (e.g. springs that follow Hooke's Law) generate restoring forces that act to maintain them in an equilibrium shape."],
      ['Physics', 1, 1, '', "PH.1.07 - Students will be able to predict an object's motion when it is subject to a restoring force"],
      ['Physics', 1, 1, '', "PH.1.08 - Students will be able to use pressure difference between two points of a fluid and Newton's laws to analyze behavior of that fluid."],
      ['Physics', 1, 1, '', "PH.1.09 - Students will be able to apply principles of fluid dynamics to determine pressure and velocity in a variety of typical fluid systems"],
      ['Physics', 1, 1, '', "PH.1.10 - Students will be able to design a system for efficient energy production using concepts of temperature, heat, and thermal energy."],
      ['Physics', 1, 1, '', "PH.1.11 - Students will be able to analyze energy flow in typical heating and cooling applications by applying the 1st Law of Thermodynamics."]
    ]
